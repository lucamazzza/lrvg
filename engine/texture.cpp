...
ENG_API Texture::Texture(const std::string path) {
    DEBUG("Loading texture from file '%s'", path.c_str());
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_BGRA, GL_UNSIGNED_BYTE, (void*) FreeImage_GetBits((FIBITMAP*) this->bitmap));
}
...
ENG_API Texture::~Texture() {
    ...
    if (this->bitmap != nullptr) FreeImage_Unload((FIBITMAP*) this->bitmap);
    else WARN("Texture bitmap is null during destruction");
}
