...
int main() {
    ...
    plane->set_position(glm::vec3(0.0f, -1.0f, 0.0f));
    plane->set_scale(glm::vec3(10.0f));
    plane->set_cast_shadows(false);
    root->add_child(plane);
    auto sphere = std::make_shared<lrvg::Sphere>();
    sphere->set_name("MySphere");
    sphere->set_position(glm::vec3(-2.0f, 0.0f, 0.0f));
    sphere->set_cast_shadows(false);
    root->add_child(sphere);
    auto cube = std::make_shared<lrvg::Cube>();
    cube->set_name("MyCube");
    cube->set_position(glm::vec3(2.0f, 0.0f, 0.0f));
